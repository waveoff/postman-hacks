...
pm.test("Update auth token (EnvVar)", function () {
    pm.environment.set("YOUR_ACCESS_TOKEN_ENV_VARIABLE_NAME", pm.response.json().YOUR_RESPONSE_JSON_AUTH_TOKEN_KEY);
    pm.environment.set("YOUR_REFRESH_TOKEN_ENV_VARIABLE_NAME", pm.response.json().YOUR_RESPONSE_JSON_REFRESH_TOKEN_KEY);
});